...
//////////////////////////////////////////////////////////////////////////////////
//MC20 各项测试(拨号测试、短信测试、GPRS测试、蓝牙测试)共用代码

//MC20发送命令后,检测接收到的应答
//str:期待的应答结果
//返回值:0,没有得到期待的应答结果
//其他,期待应答结果的位置(str的位置)
u8* MC20_check_cmd(u8 *str)
{
    char *strx=0;
    if(USART2_RX_STA&0X8000)		//接收到一次数据了
    {
...
//       1,发送失败
u8 MC20_send_cmd(u8 *cmd,u8 *Re1,u8 *Re2,u8 *Re3,u16 waittime)
{
    u8 res=0;
    USART2_RX_STA=0;
    if((u32)cmd<=0XFF)
    ...
    if((Re1&&waittime)||(Re3&&waittime)||(Re2&&waittime))		//需要等待应答
    {
        while(--waittime)	//等待倒计时
        {
            ...
            if(USART2_RX_STA&0X8000)//接收到期待的应答结果
            {
                ...
                if (MC20_check_cmd(Re1))
                {
                    return 1;
                }
                if (MC20_check_cmd(Re2))
                {
                    return 2;
                }
                if (MC20_check_cmd(Re3))
                {
                    return 3;
                }
                USART2_RX_STA=0;
...
//接收MC20返回数据（蓝牙测试模式下使用）
//request:期待接收命令字符串
//waittimg:等待时间(单位：10ms)
//返回值:0,发送成功(得到了期待的应答结果)
//       1,发送失败
//u8 MC20_wait_request(u8 *request ,u16 waittime)
//{
//    u8 res = 1;
//    u8 key;
//    if(request && waittime)
...
//        while(--waittime)
//        {
//            key=KEY_Scan(0);//这个是本来就有的吗啊？？
//            if(key==WKUP_PRES) return 2;//返回上一级
//            delay_ms(10);
//            if(USART2_RX_STA &0x8000)//接收到期待的应答结果
//            {
//                if(MC20_check_cmd(request)) break;//得到有效数据
//                USART2_RX_STA=0;
//            }
//        }
//        if(waittime==0)res=0;
...
u8 MC20_call_test(void)
{
	...
	u8 cmode=0;	//模式
        ...
        if(MC20_send_cmd("AT+CLIP=1","OK",200))return 1;	//设置来电显示
        if(MC20_send_cmd("AT+COLP=1","OK",200))return 2;	//设置被叫号码显示
	...
	while(1)
	{
		...
		if(USART2_RX_STA&0X8000)		//接收到数据
		{
			...
			if(cmode==1||cmode==2)
			{
                                if(cmode==1)if(MC20_check_cmd("+COLP:"))cmode=2;	//拨号成功
                                if(MC20_check_cmd("NO CARRIER"))cmode=0;	//拨号失败
                                if(MC20_check_cmd("NO ANSWER"))cmode=0;	//拨号失败
                                if(MC20_check_cmd("ERROR"))cmode=0;		//拨号失败
			}
                        if(MC20_check_cmd("+CLIP:"))//接收到来电
			{
				cmode=3;
                                p=MC20_check_cmd("+CLIP:");
				p+=8;
		...
		{ 
			...
			}else
			{
				...
				if(key==15)
				{
					if(cmode==3)//接收到来电
					{
                                                MC20_send_cmd("ATA","OK",200);//发送应答指令
						Show_Str(40+56,70,200,16,callbuf,16,0);
						cmode=2;
					}else
					{ 
                                                MC20_send_cmd("ATH",0,0);//不管有没有在通话,都结束通话
						break;//退出循环
					}
				}
			} 
		...
		if(oldmode!=cmode)//模式变化了
		{
			...
			if(cmode==2)Show_Str(40,90,200,16,"DTMF音:",16,0);	//通话中,可以通过键盘输入DTMF音
                        ...
                        MC20_load_keyboard(0,180,(u8**)kbd_tbl1);		//显示键盘
			oldmode=cmode; 
		}
		if((lenx%50)==0)
    {
